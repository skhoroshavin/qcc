...
TEST_P(uint_limits, test_t)
{
    ...
    ASSERT_UNSIGNED(value, <=, qcc_uint_max(sizeof(test_t)));
}
...
TEST_P(gen_uint_in_range, test_t)
{
    ...
    ASSERT_UNSIGNED(value, >=, min);
    ASSERT_UNSIGNED(value, <=, max);
}
...
TEST_P(gen_uint_array, test_t)
{
    ...
    {
        ASSERT_UNSIGNED(test.data[i], >=, min);
        ASSERT_UNSIGNED(test.data[i], <=, max);
    }
}
...
TEST_P(gen_uint_fixed_size_array, test_t)
{
    ...
    ASSERT_UNSIGNED(test.size, ==, size);
}
